<template>
  <div>
    <div class="circle">
      ...
      >
      <div v-if="isShow" id="siteBox" class="siteBox">
        <header>
          <ul class="classify">
            <li
    ...
    getData() {
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState === 4 && xhr.status === 200) {
          const locationN = JSON.parse(xhr.responseText)
          for (const key of Object.keys(locationN)) {
            for (const key2 of Object.keys(this.locationName)) {
              const pattern = new RegExp('[' + key2.toString() + ']')
              if (pattern.test(key)) {
